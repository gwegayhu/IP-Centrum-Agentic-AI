...
const CrewApiForm: React.FC<CrewApiFormProps> = ({ baseUrl, bearerToken, className }) => {
  ...
  const [documentationExperts, setDocumentationExperts] = useState('');
  const [devopsExperts, setDevopsExperts] = useState('');
  const [teamMembersWithExpertise, setTeamMembersWithExpertise] = useState('');
  const [backendExperts, setBackendExperts] = useState('');
  const [frontendExperts, setFrontendExperts] = useState('');
      ...
      const kickoffResponse = await fetch(`${baseUrl}/kickoff`, {
        ...
        body: JSON.stringify({
          inputs: {
            ...
            documentation_experts: documentationExperts,
            devops_experts: devopsExperts,
            team_members_with_expertise: teamMembersWithExpertise,
            backend_experts: backendExperts,
            frontend_experts: frontendExperts
          }
        })
      });
  ...
  return (
	...
	<label className="block text-sm font-medium text-gray-700">
		...
		<textarea
			value={documentationExperts}
			onChange={(e) => setDocumentationExperts(e.target.value)}
	...
	<label className="block text-sm font-medium text-gray-700">
		...
		<textarea
			value={devopsExperts}
			onChange={(e) => setDevopsExperts(e.target.value)}
			className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm break-words whitespace-pre-wrap"
			required
		/>
	</label>
	...
	<label className="block text-sm font-medium text-gray-700">
		...
		<textarea
			value={teamMembersWithExpertise}
			onChange={(e) => setTeamMembersWithExpertise(e.target.value)}
			className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm break-words whitespace-pre-wrap"
			required
		/>
	</label>
	...
	<label className="block text-sm font-medium text-gray-700">
		Backend Experts:
		<textarea
			value={backendExperts}
			onChange={(e) => setBackendExperts(e.target.value)}
			className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm break-words whitespace-pre-wrap"
			required
		/>
	</label>
	...
	<label className="block text-sm font-medium text-gray-700">
		Frontend Experts:
		<textarea
			value={frontendExperts}
			onChange={(e) => setFrontendExperts(e.target.value)}
			className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm break-words whitespace-pre-wrap"
			required
		/>
	</label>
